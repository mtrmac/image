...
func TestNewEphemeralGPGSigningMechanism(t *testing.T) {
	...
	require.NoError(t, err)
	mech, keyIdentities, err = NewEphemeralGPGSigningMechanism([][]byte{keyBlob})
	require.NoError(t, err)
	defer mech.Close()
	...
	require.NoError(t, err)
	mech, keyIdentities, err = NewEphemeralGPGSigningMechanism([][]byte{bytes.Join([][]byte{keyBlob, keyBlob}, nil)})
	require.NoError(t, err)
	defer mech.Close()
	...
	mech, keyIdentities, err = NewEphemeralGPGSigningMechanism([][]byte{[]byte("This is invalid")})
